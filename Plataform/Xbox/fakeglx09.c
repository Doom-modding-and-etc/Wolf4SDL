...
====================================================================================================================================
   ...
   This is FakeGLx09, a XBOX port of FakeGL.
*/
...
void D3D_InitTexture (d3d_texture_t *tex)
{
	...
	//D3D_SAFE_RELEASE (tex->teximg); // MARTY - Macro not working :/
...
//			D3D_SetRenderState (D3DRS_ZBIAS, 8); 
			D3D_SetRenderState (D3DRS_ZBIAS, 0); //HACK This is a nasty hack for Xash3D only - MARTY
		}										 //     Polygons look very glitchy if we dont do this!	
...
/*	  // MARTY - Texture coordinates don't work, re-enabled ^
      IDirect3DDevice8_Begin(d3d_Device, D3DPT_QUADLIST);
      for (i = 0; i < d3d_NumVerts; i ++)
      {
         IDirect3DDevice8_SetVertexData4f(d3d_Device, D3DVSDE_VERTEX, d3d_Vertexes[i].position.x, d3d_Vertexes[i].position.y, d3d_Vertexes[i].position.z, 1.0f);
         IDirect3DDevice8_SetVertexData4f(d3d_Device, D3DVSDE_NORMAL, d3d_Vertexes[i].normal.x, d3d_Vertexes[i].normal.y, d3d_Vertexes[i].normal.z, 1.0f);
         IDirect3DDevice8_SetVertexDataColor(d3d_Device, D3DVSDE_DIFFUSE, d3d_Vertexes[i].c);

         for (c = 0; c < D3D_MAX_TMUS; c++)
         {
            IDirect3DDevice8_SetVertexData2f(d3d_Device, D3DVSDE_TEXCOORD0, d3d_Vertexes[i].st[c].s, d3d_Vertexes[i].st[c].t);
         }
      }
      IDirect3DDevice8_End(d3d_Device);
*/
	case GL_LINES:
...
//MARTY END 
	...
	if (format == 1 || format == GL_LUMINANCE || format == GL_LUMINANCE8 ) //MARTY
		srcbytes = 1;
	...
	if (internalformat == 1 || internalformat == GL_LUMINANCE || internalformat == GL_LUMINANCE8 ) //MARTY
		dstbytes = 1;
	...
	//MARTY START - Swizzle the textures on XBox!
	rect.left = 0;
    rect.top = 0;
    rect.right = width;
    rect.bottom = height;
	...
	//Lock the texture
	IDirect3DTexture8_LockRect (texture, 0, &lr, &rect, 0);
	...
	IDirect3DTexture8_LockRect (surfaceTemp, 0, &lr2, NULL, NULL);
	...
	IDirect3DTexture8_UnlockRect (surfaceTemp, 0);
...
SubImage_t* GetSubImageCache(int iTexNum)
{
	...
	IDirect3DDevice8_CreateImageSurface (d3d_Device, 128, 128, D3DFMT_A8R8G8B8, &tex->texture);

#if 0 //Unnecessary slow down??
	IDirect3DTexture8_LockRect (tex->texture, 0, &lockedRect, NULL, NULL );	

	memset(lockedRect.pBits, 0, 128*128*lockedRect.Pitch); //MARTY FIXME Don't hardcode these values

	IDirect3DTexture8_UnlockRect (tex->texture, 0);
#endif
...
void glTexSubImage2D (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid *pixels)
{
	...
	IDirect3DTexture8_LockRect (texSelect->texture, level, &lrunswiz, NULL, NULL );	
	...
	IDirect3DTexture8_UnlockRect (texSelect->texture, level);

	IDirect3DTexture8_GetLevelDesc(d3d_TMUs[d3d_CurrentTMU].boundtexture->teximg, level, &desc);

	//Lock the texture
	IDirect3DTexture8_LockRect (d3d_TMUs[d3d_CurrentTMU].boundtexture->teximg, level, &lr, NULL, NULL);
...
void glDeleteTextures (GLsizei n, const GLuint *textures)
{
	...
	for (tex = d3d_Textures; tex; tex = tex->next)
	{
		for (i = 0; i < n; i++)
		{
			...
			{
				DeleteSubImageCache(tex->glnum); //MARTY
...
*/	{
	...
	d3d_PresentParams.MultiSampleType = D3DMULTISAMPLE_2_SAMPLES_MULTISAMPLE_LINEAR;//D3DMULTISAMPLE_NONE;
...
*/
	if (d3d_SceneBegun)
	{
		...
		IDirect3DDevice8_SetFlickerFilter(d3d_Device, 1); //MARTY
		IDirect3DDevice8_SetSoftDisplayFilter(d3d_Device, TRUE); //MARTY
...
void GL_EnableDisable (GLenum cap, BOOL enabled)
{
	...
	switch (cap)
	{
//	case GL_SCISSOR_TEST: //MARTY - d3d8 doesnt have scissor test so we'll just live without it for now.
//		break;
