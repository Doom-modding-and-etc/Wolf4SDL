...
void T_Projectile (objtype *ob)
{
    ...
    deltax = (int)FixedMul((fixed)speed,costable[ob->angle]);
    deltay = (int)-FixedMul((fixed)speed, sintable[ob->angle]);
    ...
    deltax = labs((int)ob->x - player->x);
    deltay = labs((int)ob->y - player->y);
...
void    A_StartDeathCam (objtype *ob)
{
    ...
    */
    ...
    do
    {
        xmove = (int)FixedMul(dist,costable[player->angle]);
        ymove = (int)-FixedMul(dist, sintable[player->angle]);
