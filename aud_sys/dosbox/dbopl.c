...
static dbinline void Operator__SetState(Operator *self, Bit8u s );
static dbinline Bit32u Chip__ForwardNoise(Chip *self);
...
*/
static dbinline Bits MakeVolume( Bitu wave, Bitu volume ) {
...
static dbinline void Operator__UpdateAttack(Operator *self, const Chip* chip ) {
...
static dbinline void Operator__UpdateDecay(Operator *self, const Chip* chip ) {
...
static dbinline void Operator__UpdateRelease(Operator *self, const Chip* chip ) {
...
static dbinline void Operator__UpdateAttenuation(Operator *self) {
...
static dbinline Bit32s Operator__RateForward(Operator *self, Bit32u add ) {
...
static dbinline Bitu Operator__ForwardVolume(Operator *self) {
	return self->currentLevel + (self->volHandler)(self);
}


static dbinline Bitu Operator__ForwardWave(Operator *self) {
...
static dbinline int Operator__Silent(Operator *self) {
...
static dbinline void Operator__Prepare(Operator *self, const Chip* chip )  {
...
static dbinline Bits Operator__GetWave(Operator *self, Bitu index, Bitu vol ) {
#if( DBOPL_WAVE == WAVE_HANDLER )
...
static dbinline Bits Operator__GetSample(Operator *self, Bits modulation ) {
...
static dbinline Operator* Channel__Op( Channel *self, Bitu index ) {
...
static dbinline void Channel__GeneratePercussion(Channel *self, Chip* chip,
	...
	if ( !ENV_SILENT( hhVol ) ) {
		...
		sample += (Bit32u)Operator__GetWave( Channel__Op(self, 2), hhIndex, hhVol );
	}
	/* Snare Drum */
	sdVol = (Bit32u)Operator__ForwardVolume( Channel__Op(self, 3) );
	if ( !ENV_SILENT( sdVol ) ) {
		Bit32u sdIndex = ( 0x100 + (c2 & 0x100) ) ^ ( noiseBit << 8 );
		sample += (Bit32u)Operator__GetWave( Channel__Op(self, 3), sdIndex, sdVol );
	}
	/* Tom-tom */
	sample += (Bit32u)Operator__GetSample( Channel__Op(self, 4), 0 );

	/* Top-Cymbal */
	tcVol = (Bit32u)Operator__ForwardVolume(Channel__Op(self, 5));
	if ( !ENV_SILENT( tcVol ) ) {
		Bit32u tcIndex = (1 + phaseBit) << 8;
		sample += (Bit32s)Operator__GetWave( Channel__Op(self, 5), tcIndex, tcVol );
	}
	sample <<= 1;
	if ( opl3Mode ) {
		output[0] += sample;
...
Channel* Channel__BlockTemplate(Channel *self, Chip* chip,
	...
	for ( i = 0; i < samples; i++ ) {
		...
		if ( mode == sm2AM || mode == sm3AM ) {
			sample = out0 + (Bit32s)Operator__GetSample( Channel__Op(self, 1), 0 );
			...
			sample = out0;
			next = Operator__GetSample( Channel__Op(self, 1), 0 );
			sample += (Bit32s)Operator__GetSample( Channel__Op(self, 2), next );
			sample += (Bit32s)Operator__GetSample( Channel__Op(self, 3), 0 );
		}
		switch( mode ) {
		case sm2AM:
		case sm2FM:
...
static dbinline Bit32u Chip__ForwardLFO(Chip *self, Bit32u samples ) {
	...
	*/
	...
	for ( i = 0; i < 64; i++ ) {
		...
		if ( chNum >= 12 ) {
			...
			OpOffsetTable[i] = ChanOffsetTable[ chNum ] + (Bit16u)blah;
		}
#if 0
		/* 
